...
Function isSubjectNameValid
{
    ...
    $isValid = ($cert.SubjectName.Name -match $expectedSubjectName)
...
{
    [OutputType([string])]
    $regValue = Get-ItemProperty -Path "HKLM:\Software\Microsoft\Microsoft Operations Manager\3.0\Machine Settings" -Name ChannelCertificateSerialNumber -ErrorAction Ignore
    if ($regValue -eq $Null)
    {
        if ($EnableDiagnostics)
        {
